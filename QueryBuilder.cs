    ...
    public class QueryBuilder : IDisposable
    {
        ...
        public void Create<T>(T obj) where T : IClassModel
        {
            ...
            for (int i = 0; i < propertyInfos.Length; i++)
            {
                ...
                {
                    value.Add($"\"{((DateTime)prop.GetValue(obj)).Year}-{((DateTime)prop.GetValue(obj)).Month}-{((DateTime)prop.GetValue(obj)).Day}\"");
                }
                else if (prop.PropertyType == typeof(string))
                {
                    value.Add($"\"{prop.GetValue(obj).ToString()}\"");
                }
            ...
            var command = connection.CreateCommand();
            command.CommandText = $"INSERT INTO {obj.GetType().Name} ({sbName}) VALUES ({sbValue})";
